...
def get_page_url(
    ...
    if url_view_name is not None:
       ...
       # This bit of code comes from the default django url tag
        ...
        try:
            url = reverse(url_view_name, args=url_extra_args,
                kwargs=url_extra_kwargs, current_app=current_app)
        except NoReverseMatch, e:
            if settings.SETTINGS_MODULE:
                project_name = settings.SETTINGS_MODULE.split('.')[0]
                url = reverse(project_name + '.' + url_view_name,
                    args=url_extra_args, kwargs=url_extra_kwargs,
...
class BootstrapPagerNode(template.Node):
    ...
    def render(self, context):
        ...
        for (argname, argvalue) in kwargs.items():
            if isinstance(argvalue, template.FilterExpression):
                try:
...
class BootstrapPaginationNode(template.Node):
    ...
    def render(self, context):
        ...
        if alignment not in ['left', 'center', 'right']:
            raise Exception(
                'Optional argument "alignment" expecting one of "left", '
                '"center", or "right"')
        ...
        if range_length is None:
            ...
            if range_length < 1:
                raise Exception(
                    'Optional argument "range" expecting integer greater '
                    'than 0')
...
def bootstrap_paginate(parser, token):
    ...
    if len(bits) < 2:
        raise template.TemplateSyntaxError(
            '\'%s\' takes at least one argument (Page object reference)'
             % bits[0])
...
def bootstrap_pager(parser, token):
    ...
    Named Parameters::
        ...
        previous_label - The label to show for the Previous link (defaults to
            "Previous Page")

        next_label - The label to show for the Next link (defaults to "Next
            Page")

        previous_title - The link title for the previous link (defaults to
            "Previous Page")

        next_title - The link title for the next link (defaults to "Next
            Page")
    ...
    if len(bits) < 2:
        raise template.TemplateSyntaxError(
            '%r takes at least one argument (Page object reference)'
             % bits[0])
    page = parser.compile_filter(bits[1])
    kwargs = {}
    bits = bits[2:]
